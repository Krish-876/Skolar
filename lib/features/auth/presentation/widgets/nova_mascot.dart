...
class _NovaMascotState extends State<NovaMascot>
  ...
  void initState() {
    ...
    _pupilCtrl = AnimationController(vsync: this)
      ..addListener(_onPupilTick);
    ...
    _scaleCtrl = AnimationController(vsync: this, value: 0)
      ..addListener(() {
        setState(() =>
            _eyeScale = 1.0 + _scaleCtrl.value * _scaleCtrl.value * _scaleAmount);
      });
    ...
    _pulseCtrl = AnimationController(
      ...
      duration: const Duration(milliseconds: 900),
        ...
        setState(() =>
            _pupilScale = 1.0 + sin(_pulseCtrl.value * pi) * 0.12);
      });
  ...
  void _onPupilTick() {
    final t = CurvedAnimation(parent: _pupilCtrl, curve: Curves.easeInOut).value;
    setState(() {
  ...
  void _applyState(MascotState state) {
    ...
    switch (state) {
      ...
      case MascotState.guiltyGlance:
        ...
        _glanceTimer = Timer(Duration(milliseconds: 600 + _rng.nextInt(100)), () {
          if (!mounted) return;
...
class _MascotPainter extends CustomPainter {
  ...
  const _MascotPainter({
    ...
    required this.pupilScale, required Color bodyColor, required Color highlightColor,
  });
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final h = size.height;
    canvas.drawOval(
    Rect.fromCenter(center: Offset(w * 0.5, h * 0.96), width: w * 0.55, height: h * 0.06),
    Paint()
...
final bodyPath = Path()
  ..addRRect(RRect.fromRectAndCorners(
    Rect.fromCenter(center: Offset(w * 0.5, h * 0.58), width: w * 0.85, height: h * 0.75),
...
void drawStumpyLeg(double xPos) {
  canvas.drawRRect(
    RRect.fromRectAndRadius(
      ...
      Rect.fromCenter(center: Offset(w * xPos, h * 0.94), width: w * 0.18, height: h * 0.12),
...
Paint eyeWhite(Offset center) => Paint()
  ...
  canvas.save();
  canvas.clipPath(Path()..addOval(Rect.fromCircle(center: eyeCenter, radius: eyeRadius - 1)));

  // 1. Deep Iris Glow (The magic touch)
  canvas.drawCircle(pupilCenter, pupilRadius, Paint()..color = const Color(0xFF1A1A1A));
  canvas.drawCircle(
    pupilCenter, 
    pupilRadius * 0.85, 
    Paint()..color = const Color(0xFF8B5ED4).withOpacity(0.25) // Subtle violet glow
  );
  ...
  canvas.restore();
    ...
    if (blushOpacity > 0) {
      final blushPaint = Paint()
        ...
        ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 6);
      canvas.drawOval(Rect.fromCenter(center: Offset(w * 0.24, h * 0.57), width: w * 0.18, height: h * 0.10), blushPaint);
      canvas.drawOval(Rect.fromCenter(center: Offset(w * 0.76, h * 0.57), width: w * 0.18, height: h * 0.10), blushPaint);
    }
  }

  /// Fills the eye with a lid colour from the top down.
  /// [amount] 0 = nothing, 1 = fully covered.
  void _drawEyelid(Canvas canvas, Offset eyeCenter, double eyeRadius, double amount) {
    canvas.save();
    canvas.clipPath(
      Path()..addOval(Rect.fromCircle(center: eyeCenter, radius: eyeRadius + 0.5)),
    );
